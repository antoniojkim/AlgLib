...
def median2list_naive(nums1: List[int], nums2: List[int]) -> float:
    '''
    Naive Solution where the two lists are merged and the median is easily found from the merged list
...
def median2list(nums1: List[int], nums2: List[int]) -> float:
    ...
    while (min_i <= max_i):
        ...
        if (i < len(nums1) and nums1[i] < nums2[j-1]):
            ...
            if (size%2) != 0: return max_left
            
            if   i == len(nums1): min_right = nums2[j]
            elif j == len(nums2): min_right = nums1[i]
...
def assert_equals(inputs):
    '''
    Asserts that the solution for the better solution matches the solution for the naive solution
    '''
